...
const inputSearchMobile = $('.heading__history__search');
...
const app = {
    handle: function() {
        ...
        inputSearchMobile.onkeypress = function(e) {
            if(e.keyCode === 32) {
                var valueSearch = inputSearchMobile.value.toLowerCase();
                dataListFilms(films => {
                    // Get listFilm thông qua keySearch
                    var listNameFilm = films.reduce((names, film) => {
                        if(film.nameVi.toLowerCase().indexOf(valueSearch) !== -1 
            ...
            if(e.keyCode === 13) {
                window.location.href = 
                    'http://127.0.0.1:5500/assets/html/filter_film.html?search=' + handleAccentedString(inputSearchMobile.value);
            } else {
                return;
            }
        };
